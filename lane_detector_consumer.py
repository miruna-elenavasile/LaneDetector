...
import cv2
import numpy as np
from object_socket import ObjectReceiver

# Initializam receiver-ul
receiver = ObjectReceiver(ip='127.0.0.1', port=5000)
print("[CONSUMER] Pornire server... Astept producatorul sa trimita cadre.")
receiver.start()
print("[CONSUMER] Producator conectat! Incepe procesarea Lane Detection.")

THRESHOLD = 190
MIN_POINTS = 10

# Istoric pentru netezirea mișcării (Moving Average)
...
while True:
    # Primim cadrul de la video_producer.py prin retea
    frame = receiver.get_object()

    # Daca am primit None sau semnal de oprire, iesim din bucla
    if frame is None:
        break

    # --- Algoritmul de Lane Detection ---
    ...
    cv2.imshow("Final Lane Detection - Socket Stream", final_frame)
